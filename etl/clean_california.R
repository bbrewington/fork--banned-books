...
#   Title:     chars  1 – AUTH_START-1        (~22 chars)
#   Author:    chars  AUTH_START – pub_start-1 (~19-21 chars, varies with page)
#   Publisher: chars  pub_start  – reason_start-1
#   Reason:    chars  reason_start – end
#
# reason_start is detected per page from "Title 15, Section" line positions.
# pub_start is derived as reason_start - 28 (verified across pages).
# AUTH_START is fixed (consistent across all pages).
# ---------------------------------------------------------------------------
AUTH_START <- 23L
...
pub_start    <- reason_start - 28L

for (page_num in seq_along(raw_pages)) {
  lines <- strsplit(raw_pages[[page_num]], "\n")[[1]]

  # Calibrate reason_start (and derived pub_start) from "Title 15, Section" lines on this page
  ...
  for (line in lines) {
    ...
    title_part  <- str_trim(substr(line, 1L,           min(n, AUTH_START - 1L)))
    author_part <- str_trim(substr(line, AUTH_START,   min(n, pub_start - 1L)))
    pub_part    <- str_trim(substr(line, pub_start,    min(n, reason_start - 1L)))
